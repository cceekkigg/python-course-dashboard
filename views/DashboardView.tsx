...
const DashboardView: React.FC<DashboardViewProps> = ({ user, onLogout, students, onUpdateStudents, onUpdateUser }) => {
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
      ...
      try {
          ...
          setNewPassword('');
          setTimeout(() => { setIsSettingsOpen(false); setPasswordSuccess(''); }, 1500);
      } catch (err: any) {
      ...
      <aside className="w-full bg-slate-900 text-white lg:h-screen lg:w-64 lg:fixed lg:left-0 lg:top-0 lg:overflow-y-auto z-10 flex flex-col">
        ...
        <nav className="flex-1 space-y-1 px-3 py-6">
          {navItems.map((item) => {
             ...
             return (
                <button key={item.id} onClick={() => setCurrentView(item.id)} className={`w-full group flex items-center rounded-lg px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? 'bg-blue-600 text-white shadow-md' : 'text-slate-300 hover:bg-slate-800 hover:text-white'}`}>
        ...
        <div className="p-4 border-t border-slate-800 bg-slate-900">
           <div className="rounded-xl bg-slate-800 p-3">
              ...
              <button onClick={onLogout} className="flex w-full items-center justify-center rounded-lg border border-slate-600 bg-transparent px-2 py-1.5 text-xs font-medium text-slate-300 hover:bg-slate-700 hover:text-white transition-colors">
