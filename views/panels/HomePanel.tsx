...
export const HomePanel: React.FC<HomePanelProps> = ({ user, announcements }) => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      if (data) {
         const completedExercises = data.filter(d => d.assignment_id.startsWith('ex-') && d.status === 'submitted').length;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl bg-white p-6 shadow-sm border border-slate-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-bold text-slate-900">{stats.score}/100</p>
          </div>
      ...
      {/* Carousel & Activity (Same as before) */}
        ...
        <div className="rounded-xl bg-white shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div className="p-6">
             <div className="flex items-start space-x-4 mb-4">
                <div className="bg-yellow-100 text-yellow-800 text-xs font-bold px-2 py-1 rounded text-center min-w-[60px]">DEC 8</div>
                <div>
