...
import { Lock, FileText, FileSpreadsheet, Presentation, Link as LinkIcon, Trash2, Plus, Loader2, Eye, ExternalLink, Download, X } from 'lucide-react';
...
export const MaterialsPanel: React.FC<MaterialsPanelProps> = ({ user }) => {
  ...
  // 1. Fetch Materials & Structure
  const fetchData = async () => {
    try {
        // Fetch Materials
        const { data: matData } = await supabase.from('materials').select('*');
        if (matData) setDynamicMaterials(matData as Material[]);

        // Fetch Structure (Weeks + Days)
        ...
        // Sort days
        const sorted = (weekData || []).map((w: any) => ({
            ...w,
            days: (w.days || []).sort((a: any, b: any) => a.day_index - b.day_index)
        }));
        setCourseWeeks(sorted as CourseWeek[]);
    } catch (err) {
    ...
    <div className="space-y-6 animate-fade-in relative">
      ...
      <div className="space-y-8">
        {courseWeeks.map((week) => {
           const guestLock = isGuest && week.week_number !== 1; // Unlock only Week 1 for guests
           const isLocked = week.is_locked || guestLock;
            ...
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl h-[80vh] flex flex-col overflow-hidden">
                <div className="flex justify-between items-center p-4 border-b border-slate-100 bg-slate-50">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { setPreviewMaterial(null); setCsvContent([]); }} className="p-1.5 hover:bg-slate-200 rounded-full transition-colors text-slate-500">
                            <X className="w-6 h-6" />
                        </button>
                    </div>
                </div>
                <div className="flex-1 bg-slate-100 overflow-auto p-4 flex items-center justify-center">
                    {previewMaterial.type === 'pdf' && (
                        <iframe src={`${previewMaterial.url}#toolbar=0`} className="w-full h-full rounded-lg shadow-sm bg-white" title="PDF Preview" />
                    )}
                                    ...
                                    <thead>
                                        <tr>
                                            {csvContent[0].map((header, i) => (
                                                <th key={i} className="border-b-2 border-slate-200 p-2 font-bold text-slate-700 bg-slate-50 sticky top-0">{header}</th>
                                            ))}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {csvContent.slice(1).map((row, i) => (
                                            <tr key={i} className="hover:bg-slate-50 border-b border-slate-100">
                                                {row.map((cell, j) => (
                                                    <td key={j} className="p-2 text-slate-600 border-r border-slate-100 last:border-0 truncate max-w-[200px]">{cell}</td>
                                                ))}
                                            </tr>
                                        ))}
                                    </tbody>
