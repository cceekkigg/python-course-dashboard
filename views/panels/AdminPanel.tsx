...
import { Search, Edit2, Users, Bell, Activity, Globe, X, RefreshCw, Upload, AlertTriangle, BookOpen, Lock, Unlock, Trash2, Calendar, Save } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ students, onUpdateStudents, announcements, onAddAnnouncement, onDeleteAnnouncement, onUpdateAnnouncement }) => {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
            <div className="flex justify-between items-center bg-slate-50 p-3 rounded-lg border border-slate-200">
               <div className="text-sm text-slate-600 flex items-center gap-2">
                   ...
                   <span><strong>Note:</strong> Homework scores are read-only. Attendance changes must be saved.</span>
               </div>
               {pendingSaves.size > 0 && (
                   <Button size="sm" onClick={saveAttendance} disabled={isSavingScores} className="bg-blue-600 hover:bg-blue-700 text-white">
                            ...
                            return (
                                <tr key={student.id} className="hover:bg-slate-50">
                                    ...
                                    {homeworkIds.map(hwId => (
                                        <td key={hwId} className="px-2 py-3 text-center border-r border-slate-100 font-mono text-slate-600">
      ...
      {activeTab === 'materials' && (
        <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm max-w-lg"><h3 className="font-bold mb-4 flex items-center gap-2"><Upload className="w-5 h-5 text-blue-600" /> Upload Course Material</h3><form onSubmit={handleFileUpload} className="space-y-4"><div><label className="block text-sm font-medium mb-1 text-slate-700">Select Day</label><select className="w-full border border-slate-300 rounded-lg p-2.5 text-sm bg-white" value={selectedDay} onChange={(e) => setSelectedDay(e.target.value)}>{adminWeeks.map(week => (<optgroup key={week.id} label={`Week ${week.week_number}: ${week.title}`}>{week.days.map(day => <option key={day.id} value={day.id}>{day.title}</option>)}</optgroup>))}</select></div><div><label className="block text-sm font-medium mb-1 text-slate-700">File</label><div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:bg-slate-50 transition-colors"><input type="file" id="file-upload" onChange={(e) => setFile(e.target.files?.[0] || null)} className="hidden" /><label htmlFor="file-upload" className="cursor-pointer">{file ? <div className="text-blue-600 font-medium break-all">{file.name}</div> : <div className="text-slate-500 text-sm">Click to select a file <br/> <span className="text-xs text-slate-400">(PDF, CSV, Slides)</span></div>}</label></div></div><Button type="submit" fullWidth disabled={uploading || !file}>{uploading ? 'Uploading...' : 'Upload & Save'}</Button></form></div>
      )}
